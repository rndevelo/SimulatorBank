...
class AuthRepositoryImplTest {
    ...
    @Test
    fun `login with valid credentials - remote success - should save token and return User`() =
        runTest {
            // Given
    ...
    @Test
    fun `login with invalid credentials - remote returns AuthException - should propagate failure`() =
        runTest {
            // Given
    ...
    @Test
    fun `login success from remote - but saveToken fails - should propagate failure from saveToken`() =
        runTest {
            // Given
            val username = "testuser"
            val password = "password123"
            val userDto = UserDto(id = "user1", username = username, name = "Test User")
            val authDto = AuthDto(
                accessToken = "token_that_will_fail_saving",
    ...
    @Test
    fun `login success from remote - but DTO to domain mapping fails (e_g_, NPE) - should propagate failure`() =
        runTest {
            // Given
            val username = "testuser"
            val password = "password123"
            ...
            val faultyAuthDto =
                AuthDto(accessToken = "valid_token", refreshToken = "refresh", user = null)
            ...
            assertTrue(result.isFailure)
            assertTrue(
                "Expected NullPointerException due to null UserDto",
                result.exceptionOrNull() is NullPointerException
            )
        }

    @Test
    fun `login failure - data source returns non-AuthException Throwable - should wrap in AuthException UnknownError`() =
        runTest {
            // Given
            val username = "testuser"
            val password = "password"
            val causeThrowable = AuthException.InvalidCredentials
            ...
            assertEquals(causeThrowable.message, exception?.message)
        }
}
